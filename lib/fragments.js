...
// -> { init, fragments: Uint8Array[], starts: seconds[] | null, duration, qualities, dropped }
//
...
export function assembleTrack(chunks, mime) {
  ...
  if (!kept.length) {
    ...
    return { init, fragments: kept.map((f) => f.bytes), starts: null, duration: null, qualities: 1, dropped: frags.length - kept.length };
  }
  ...
  return {
    init,
    fragments,
    starts: kept.map((f) => f.start),
    duration,
    qualities: used.length,
    dropped: frags.length - kept.length,
  };
}
...
const moovParts = (init) => {
  const moov = mp4Boxes(init).find((b) => b.type === 'moov');
  return moov ? children(init, moov.start, moov.end).map((c) => ({ type: c.type, bytes: init.subarray(c.start, c.end) })) : [];
};

// A copy of a standalone trak/trex with its track_ID set (and, for a trak, its duration in
// the movie timescale).
function renumbered(box, id, movieTicks) {
  const out = box.slice();
  if (fourcc(out, 4) === 'trex') {
    w32(out, 12, id);
    return out;
  }
  const tkhd = children(out, 0, out.length).find((c) => c.type === 'tkhd');
  if (tkhd) {
    const v1 = out[tkhd.start + 8] === 1;
    w32(out, tkhd.start + (v1 ? 28 : 20), id);
    if (movieTicks != null) {
      if (v1) w64(out, tkhd.start + 36, movieTicks);
      else w32(out, tkhd.start + 28, Math.min(movieTicks, 0xffffffff));
    }
  }
  return out;
}

// A fragment's tfhd pointed at track `id`, its mfhd at sequence `seq`.
function refragment(frag, id, seq) {
  const out = frag.slice();
  const moof = mp4Boxes(out).find((b) => b.type === 'moof');
  if (!moof) return out;
  const mfhd = find(out, moof, 'mfhd');
  if (mfhd) w32(out, mfhd.start + 12, seq);
  for (const traf of children(out, moof.start, moof.end).filter((c) => c.type === 'traf')) {
    const tfhd = find(out, traf, 'tfhd');
    if (tfhd) w32(out, tfhd.start + 12, id);
  }
  return out;
}

// Can two assembled tracks be muxed: both fragmented MP4 with one track and timestamps.
export function muxable(video, audio) {
  const one = (t) => t.starts && moovParts(t.init).filter((p) => p.type === 'trak').length === 1 && moovParts(t.init).some((p) => p.type === 'mvex');
  return one(video) && one(audio);
}

// Two assembled fMP4 tracks (from assembleTrack) -> one MP4 file: track 1 video, track 2
// audio, fragments interleaved by time.
export function muxMp4(video, audio) {
  const v = moovParts(video.init);
  const a = moovParts(audio.init);
  const part = (list, type) => list.find((p) => p.type === type)?.bytes;
  const trexOf = (list) => {
    const mvex = part(list, 'mvex');
    const trex = mvex && children(mvex, 0, mvex.length).find((c) => c.type === 'trex');
    return trex ? mvex.subarray(trex.start, trex.end) : null;
  };

  const mvhd = part(v, 'mvhd').slice();
  const v1 = mvhd[8] === 1;
  const movieScale = u32(mvhd, v1 ? 28 : 20);
  const seconds = Math.max(video.duration || 0, audio.duration || 0);
  if (v1) w64(mvhd, 32, Math.round(seconds * movieScale));
  else w32(mvhd, 24, Math.min(Math.round(seconds * movieScale), 0xffffffff));
  w32(mvhd, mvhd.length - 4, 3); // next_track_ID

  const moov = makeBox(
    'moov',
    mvhd,
    renumbered(part(v, 'trak'), 1, Math.round((video.duration || 0) * movieScale)),
    renumbered(part(a, 'trak'), 2, Math.round((audio.duration || 0) * movieScale)),
    makeBox('mvex', renumbered(trexOf(v), 1), renumbered(trexOf(a), 2)),
    ...v.filter((p) => !['mvhd', 'trak', 'mvex'].includes(p.type)).map((p) => p.bytes),
  );
  const ftyp = mp4Boxes(video.init).find((b) => b.type === 'ftyp');

  const timeline = [
    ...video.fragments.map((f, i) => ({ f, t: video.starts[i], id: 1 })),
    ...audio.fragments.map((f, i) => ({ f, t: audio.starts[i], id: 2 })),
  ].sort((x, y) => x.t - y.t || x.id - y.id);
  const entries = timeline.map((e, i) => ({ bytes: refragment(e.f, e.id, i + 1), t: e.t, ref: e.id === 1 }));
  return concat([
    ftyp ? video.init.subarray(ftyp.start, ftyp.end) : new Uint8Array(0),
    moov,
    segmentIndex(entries, 1, mp4SecondsPerTick(video.init), seconds),
    ...entries.map((e) => e.bytes),
  ]);
}

// A segment index (sidx) over fragments laid out one after another: one reference per
// fragment of track `trackId`, each spanning up to the next one (fragments of other tracks
// in between included). Without it players cannot seek in a fragmented file.
//   entries: [{ bytes, t (seconds), ref (true for the indexed track) }] in file order
function segmentIndex(entries, trackId, secondsPerTick, totalSeconds) {
  const refs = [];
  for (let i = 0; i < entries.length; i++) {
    if (!entries[i].ref && refs.length) {
      refs[refs.length - 1].size += entries[i].bytes.length;
      continue;
    }
    refs.push({ t: entries[i].t, size: entries[i].bytes.length });
  }
  if (!refs.length) return new Uint8Array(0);
  const ticks = (s) => Math.round(s / secondsPerTick);
  const body = new Uint8Array(4 + 4 + 4 + 8 + 8 + 2 + 2 + refs.length * 12);
  body[0] = 1; // version 1: 64-bit times
  w32(body, 4, trackId);
  w32(body, 8, Math.round(1 / secondsPerTick));
  w64(body, 12, ticks(refs[0].t));
  w64(body, 20, 0); // first_offset: fragments start right after the sidx
  body[30] = (refs.length >> 8) & 255;
  body[31] = refs.length & 255;
  refs.forEach((r, i) => {
    const end = i + 1 < refs.length ? refs[i + 1].t : totalSeconds;
    const o = 32 + i * 12;
    w32(body, o, r.size & 0x7fffffff); // reference_type 0: media
    w32(body, o + 4, Math.max(0, ticks(end) - ticks(r.t)));
    w32(body, o + 8, 0x90000000); // starts_with_SAP, SAP type 1
  });
  return makeBox('sidx', body);
}

// A single assembled fMP4 track as a file, with a segment index so it can be seeked.
export function indexedMp4(track) {
  const scale = mp4SecondsPerTick(track.init);
  if (!track.starts || !scale) return concat([track.init, ...track.fragments]);
  const entries = track.fragments.map((bytes, i) => ({ bytes, t: track.starts[i], ref: true }));
  return concat([track.init, segmentIndex(entries, trackId(track.init), scale, track.duration), ...track.fragments]);
}
