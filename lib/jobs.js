...
// Downloads independent files (a page's images) a few at a time. One that fails does not stop
// the others: resolves to a Blob or an Error per URL, in order. Progress counts files.
export async function fetchEach(urls, { fetchImpl, signal, onProgress = () => {}, gate }) {
  ...
  const worker = async () => {
    for (;;) {
      await gate?.wait();
      if (next >= urls.length || signal?.aborted) return;
      const i = next++;
      try {
        results[i] = await fetchFile(urls[i], { fetchImpl, signal, gate });
