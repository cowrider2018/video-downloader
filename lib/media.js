...
// Images smaller than this are icons, sprites, spacers and tracking pixels.
export const MIN_IMAGE_SIZE = 10 * 1024;

// Raster formats only: SVGs on a page are almost always icons and logos.
const IMAGE_EXTS = new Set(['jpg', 'jpeg', 'png', 'gif', 'webp', 'avif', 'bmp', 'jfif']);
...
// Returns { kind: 'file' | 'hls' | 'dash' | 'image', ext } or null.
export function classify(url, mime, size) {
  ...
  // An image extension counts unless the server says the answer is something else.
  const looseType = !type || type.startsWith('image/') || type === 'application/octet-stream';
  if (IMAGE_MIME_EXT[type] || (IMAGE_EXTS.has(ext) && looseType)) {
    if (size != null && size < MIN_IMAGE_SIZE) return null;
    const known = IMAGE_EXTS.has(ext) ? (ext === 'jpeg' || ext === 'jfif' ? 'jpg' : ext) : '';
    return { kind: 'image', ext: IMAGE_MIME_EXT[type] || known || 'jpg' };
  }
...
// Images keep their own names, gathered in a folder named after the page:
// "<title>/<name>.<ext>".
export function imageFilename(title, url, ext) {
  const folder = cleanName(title) || 'images';
  const shown = displayName(url);
  const dot = shown.lastIndexOf('.');
  const bare = dot > 0 && IMAGE_EXTS.has(shown.slice(dot + 1).toLowerCase()) ? shown.slice(0, dot) : shown;
  const name = cleanName(bare) || 'image';
